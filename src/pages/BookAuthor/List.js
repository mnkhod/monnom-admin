...
const List = props => {
  ...
  const initData = booksData => {
    let tempInitialData = booksData.map(d => {
      return {
        ...
        type: d.type,
        audio_book_price:
          (d.audio_book_price != null) != 0 ? d.audio_book_price : `Үнэгүй`,
        online_book_price:
          (d.online_book_price != null) != 0 ? d.online_book_price : `Үнэгүй`,
  ...
  return (
    <React.Fragment>
      <Row>
        {modal && edit_book_id != null && (
          <UpdateBook
            book_id={edit_book_id}
            modal={modal}
            setModal={setModal}
            authors={props.available_authors}
            categories={props.available_categories}
          />
        )}
