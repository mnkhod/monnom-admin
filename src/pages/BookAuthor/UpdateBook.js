...
export default function UpdateBook(props) {
  ...
  const [audio_book_files_for_delete, set_audio_book_files_for_delete] =
    useState([])
  ...
  const updateCoverPic = async successAlert => {
        ...
        getAudioFileDuration(audio_book_files_for_save[index])
          .then(res => {
            ...
            tempFormData.append(
              ...
              audio_book_files_for_save[index].name
            )
        ...
        axios
          .all(
            tempAudioRequests.map(tempRequest =>
              axios.post(tempRequest.url, tempRequest.formData, conf)
            )
          )
          .then(() => {
            axios({
  ...
  const removePDF = async successAlert => {
    await axios({
      url: `${process.env.REACT_APP_STRAPI_BASE_URL}/books/${props.book_id}`,
      method: "PUT",
      config,
  ...
  const updatePictureComment = async successAlert => {
  ...
  const removeReference = async successAlert => {
  ...
  const deleteChapters = async successAlert => {
    let delete_requests = audio_book_files_for_delete.map(
      id => `${process.env.REACT_APP_STRAPI_BASE_URL}/book-audios/${id}`
    )

    await axios
      .all(
        delete_requests.map(request_url => axios.delete(request_url, config))
      )
      .then((...res) => {
        axios({
          url: `${process.env.REACT_APP_STRAPI_BASE_URL}/books/${props.book_id}`,
  ...
  const updateBook = async () => {
    ...
    if (!flag) {
      ...
      if (
        check_update_field.name ||
        check_update_field.intro ||
        check_update_field.youtube_url ||
        check_update_field.book_price ||
        check_update_field.ebook_price ||
        check_update_field.audio_price ||
      ...
      .then(res => {
        ...
        set_edit_book_name(res.data.name)
        {
          res.data.pdf_book_path != null
            ? set_edit_has_pdf(true)
            : set_edit_has_pdf(false)
        }
        {
          res.data.book_audios.length != 0
            ? set_edit_has_mp3(true)
            : set_edit_has_mp3(false)
        }
        set_edit_has_pdf(res.data.has_pdf)
        set_edit_has_mp3(res.data.has_audio)
        set_edit_has_sale(res.data.has_sale)
        setCoverImage(
          `${process.env.REACT_APP_STRAPI_BASE_URL}${res.data.picture.url}`
        )
        set_send_cover_pic(
          `${process.env.REACT_APP_STRAPI_BASE_URL}${res.data.picture.url}`
        )
        ...
        set_audio_book_files(getOldItems(res.data.book_audios))
        // set_referenceFiles(res.data.picture_comment)
  ...
  function handleMulti_book_author(selectedMulti) {
    if (!check_update_field.authors)
      set_check_update_field({ ...check_update_field, authors: true })
    setselectedMulti_author(selectedMulti)
  }

  function handleMulti_book_category(selectedMulti) {
    if (!check_update_field.categories)
      set_check_update_field({ ...check_update_field, categories: true })
  ...
  const uploadBook = e => {
    ...
    if (!check_update_field.pdf_file)
      set_check_update_field({ ...check_update_field, pdf_file: true })
  ...
  const uploadAudioBook = e => {
    ...
    if (!check_update_field.audio_file)
      set_check_update_field({ ...check_update_field, audio_file: true })
    ...
    audio_book_files.forEach(audio => newAudioBook.push(audio))
    set_audio_book_files(newAudioBook)
  }

  // upload hiij bga mp3 file aa ustgah
  const removeAudioBookFiles = f => {
    set_audio_book_files_for_delete(prevState => [
      ...audio_book_files_for_delete,
      f.book_id,
    ])
                  ...
                  <ul className="twitter-bs-wizard-nav nav nav-pills nav-justified">
                    ...
                    <NavItem>
                      ...
                      >
                        ...
                        <p className="my-auto">
                          {book_comments_pic.length == 0 ? `Ишлэл` : `Бататгах`}
                        </p>
                      </NavLink>
                    </NavItem>
                  </ul>
                              ...
                              <>
                                ...
                                >
                                  ...
                                  >
                                    ...
                                    {console.log(book_files)}
                                    {/* {/* {book_files.name != undefined &&
                                      book_files.name} */}
                                    {book_files[0].name != undefined &&
                                      book_files[0].name}{" "}
                                  </p>
                                  <i
                                    className="dripicons-cross font-size-20 my-auto text-dark"
                                    onClick={removeBookFiles.bind(
                                      this,
                                      book_files
                                    )}
                                    style={{
                              ...
                              <div className="text-center">
                                ...
                                <div>
                                  ...
                                  <p className="text-muted">
                                    Шинэчлэлт хийхийн тулд "Хадгалах" товчийг
                                    дарна уу ?
                                  </p>
                                </div>
                              </div>
                                        ...
                                        <div className="p-2">
                                          <Row className="align-items-center">
                                            <Col className="">
                                              <Link
                                                to="#"
                                                className="text-muted font-weight-bold d-flex justify-content-around"
                                                style={{ cursor: "default" }}
                                              >
                                                <p className="w-60 d-block">
                                                  {f.name.slice(0, 100)}
                                                </p>
                                                <i
                                                  onClick={() => {
                                                    removeRef(f.id)
                                                    if (
                                                      !check_update_field.remove_reference
                                                    ) {
                                                      set_check_update_field({
                                                        ...check_update_field,
