...
export default function BlogSingle(props){
    ...
    const handleDelete = async () => {
        ...
        try{
            const response = await axios.delete(`${process.env.REACT_APP_STRAPI_BASE_URL}/blogs/${blog.id}`);
            console.log('replace');
            history.goBack();
        }catch(e){
            setErrorMsg('Алдаа гарлаа');
    ...
    const actions = useMemo(() => {
        ...
        return (
            <div className="d-flex gap-3 justify-content-end">
                <Button color="danger" onClick={() => setIsDelete(true)}>Устгах</Button>
            </div>
        )
    }, [blog]);
                ...
                <Container fluid>
                    <Row>
                        ...
                        <Row>
                            <Col>
                                <div className="App">
                                    <CKEditor editor={ClassicEditor} data={richTextContent} onChange={handleRichTextChange}
                                        config={{
                                            extraPlugins: [
                                                CustomUploadPlugin
                                            ],
                                            toolbar: {
                                                items: [
                                                    ...
                                                    'intent', 'outtent', '|',
                                                    'embed', '|',
                                                    'htmlEmbed', 'highlight', 'image', 'imageCaption', 
                                                    'imageResize',
                                                    'ImageStyle', '|',
                                                    'imageToolbar',
                                                    'imageInsert',
                        ...
                        <Row className="mt-3 mb-5">
                            <Button color="primary" onClick={handleSubmit} className="text-white" href="#">{blog? 'Хадгалах':'Мэдээнд нэмэх'}</Button>
                        </Row>
                </Container>
            </div>
        </>
